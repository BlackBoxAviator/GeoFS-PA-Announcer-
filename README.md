# GeoFS-PA-Announcer-
GeoFS PA Announcer
A lightweight in-game Passenger Address (PA) system addon for GeoFS. Play realistic cabin announcements during your flights with a single keypress — no typing required.

# Features
Preset announcements — One-click buttons for the most common PA moments: boarding, cruise, descent, landing, turbulence, and a captain's message
Real text-to-speech — Uses your browser's built-in Web Speech API to convert announcements into actual spoken audio
Voice controls — Adjust speech rate and pitch, and choose from any voice installed on your system
ON AIR indicator — The panel shows a live green status while an announcement is playing
Stop anytime — A stop button appears during broadcast so you can cut the announcement immediately
Keyboard shortcut — Toggle the panel open and closed without touching the aircraft controls
Non-intrusive — The panel floats in the top-right corner and doesn't block your view of the sim


# Installation
Option A — Console (no extensions needed)

Open GeoFS and load into a flight
Press F12 to open your browser's DevTools
Go to the Console tab
Paste the entire contents of geofs-pa-announcer-console.js and press Enter

The PA panel will appear immediately. Running the snippet again will remove the addon cleanly.
Option B — Userscript (auto-loads every session)

Install Tampermonkey for your browser
Open the Tampermonkey dashboard and click + to create a new script
Delete the default template and paste the entire contents of geofs-pa-announcer.user.js
Press Ctrl+S to save
Load GeoFS — the addon will inject automatically on every visit


# Usage
To open the PA system, use the keyboard shortcut Shift + J

Rate — Controls how fast the voice speaks. Lower values sound more like a calm cabin crew announcement
Pitch — Adjusts the tone of the voice. Slightly below 1.0 gives a more authoritative sound
Voice — Select any voice installed on your system. English voices are listed first

# Requirements
A modern browser (Chrome, Firefox, Edge)
GeoFS loaded at geo-fs.com
Web Speech API support (available in all major browsers)


# Notes

The Web Speech API uses voices installed locally on your device. The available voices will vary depending on your operating system and browser
The addon does not interfere with GeoFS aircraft controls. All interactions are mouse-click only, with no keyboard input required while flying
